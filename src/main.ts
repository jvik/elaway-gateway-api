...
async function getChargePointData() {
  const storedToken = loadTokens();
  const response = await axios.get("https://no.eu-elaway.charge.ampeco.tech/api/v1/app/personal/charge-points/ZPR089832", {
    headers: {
...
function saveTokens(tokenResponse: ElawayTokenResponse): StoredElawayToken {
  const expiresAt = Date.now() + tokenResponse.expires_in * 1000; // Beregn utløpstidspunktet
...
async function authenticate(): Promise<ElawayTokenResponse | null> {
...
async function validateCredentials(): Promise<ElawayTokenResponse | null> {
  const storedToken = loadTokens();
...
const validCredentials = validateCredentials();
if (!validCredentials) {
  throw new Error("Kunne ikke hente gyldige autentiseringsdata");
}


// Express endpoint to get charge point data
app.get('/charger', async (_req, res) => {
  res.send(await getChargePointData())
});
